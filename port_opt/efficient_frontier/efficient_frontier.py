...
class EfficientFrontier(base_optimizer.BaseConvexOptimizer):
    """
    An EfficientFrontier object (inheriting from BaseConvexOptimizer) contains multiple optimization methods that can be called with various parameters. Including:
    - Inputs:
        - 'n_assets': number of assets
        - 'tickers': list of tickers
        - 'bounds': minimum and maximum weight of each asset OR single min/max pair if all identical OR pairs of arrays corresponding to lower/upper bounds
    ...
    def _validate_cov_matrix(cov_matrix):
        if cov_matrix is None:
            raise ValueError("Covariance matrix is None; covariance matrix must be provided")
        elif isinstance(cov_matrix, pd.DataFrame):
            return cov_matrix.values
        elif isinstance(cov_matrix, np.ndarray):
            return cov_matrix
        else:
            raise TypeError("Covariance matrix is not a dataframe or array")
    ...
    def _validate_returns(self, returns):
        ...
        if self.expected_returns is not None:
            if returns_df.shape[1] != len(self.expected_returns):
                raise ValueError(
                    "returns columns do not match expected returns. Please check your tickers."
                )
        return returns_df
    
    def _make_weight_sum_constraint(self, is_market_neutral: bool = False):
        """
        Helper method to make the weight sum constraint. If market neutral, validate the weights provided in the constructor.
        """

        if is_market_neutral:
            portfolio_possible = np.array(self._lower_bounds < 0)
    ...
    def _max_returns(self, return_value=True):
        """
